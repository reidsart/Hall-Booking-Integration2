...
class HBI_Invoices {
    ...
    public function __construct() {
        ...
        add_action( 'admin_post_hbi_approve_invoice', array( $this, 'handle_admin_approve_invoice' ), 10 );
...
if (!empty($custom_start) && !empty($custom_end)) {
        ...
        jQuery(function($){
            ...
            $('#hbi-save-invoice-main').on('click', function(e){
                ...
                $wrap.find('table.hbi-items-table tbody tr').each(function(){
                    ...
                    var subtotal = $row.find('td.hbi-subtotal').text();
                    items.push({
                        category: cat,
                        label: label,
                        quantity: qty,
    ...
    /**
     * Render Invoice Summary meta box (side)
     */
    ...
    /**
     * AJAX handler: save invoice fields & items
     */
    ...
    /**
     * Approve handler — generates PDF and emails it to customer
     */
